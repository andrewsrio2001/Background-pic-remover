...
import {
    ...
    P as Me,
    h as Ze
} from "../chunks/singletons.fe2b939d.js";
...
function Ot(t, r) {
    ...
    x || (x = Date.now(), history.replaceState({ ...history.state,
        [M]: x
    }, "", location.href));
    const fe = J[x];
    ...
    async function X({
        ...
        for (let y = 0; y < Math.max(n.length, d.branch.length); y += 1) {
            const w = n[y],
                O = d.branch[y];
            (w == null ? void 0 : w.data) !== (O == null ? void 0 : O.data) && (L = !0), w && (v = { ...v,
                ...w.data
    ...
    return {
        ...
        _start_router: () => {
            ...
            history.scrollRestoration = "manual", addEventListener("beforeunload", n => {
                ...
                C && (C = !1, history.replaceState({ ...history.state,
                    [M]: ++x
                }, "", location.href))
            });
